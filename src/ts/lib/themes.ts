 ...
 * @returns 
 */
function generateCSSCode(themeName: string){
    let cssCode = `
/* generated by src/ts/lib/themes.ts generateCSSCode() function */
    ...
    elements.forEach((element: string) => {
        const selector = theme[element].selector;
        const properties:string = [...Object.keys(theme[element].properties)]
                                        .map((property:string) => {
                                            return `${property}: ${theme[element].properties[property]};\n`
                                        })
                                        .join('');
 ...
 * @param themeName name of theme
 */
export function updateThemeCSS(themeName: string){
    // Checks if style tag already exists
    const themeCSS = document.head.querySelector('[data-style-origin="json2html"]');

    // generated css code
    const cssCode = generateCSSCode(themeName);
    ...
    if(themeCSS){
        ...
        style.type = 'text/css';
        style.setAttribute('data-style-origin', 'json2html');
